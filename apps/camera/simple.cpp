 ...
 * @brief 旋转场景
 * 
 * @details 各顶点绕y轴旋转一定角度
 * @param v 
 * @param a 
 * @return vec3 
 */
vec3 rot(const vec3& v, const double& a) {
    mat<3, 3> Ry = {{
...
int main() {
    ...
    for(int i = 0; i < model.nfaces(); i ++) {
        vec3 a = fit(persp(rot(model.vert(i, 0), M_PI / 6)));
        vec3 b = fit(persp(rot(model.vert(i, 1), M_PI / 6)));
        vec3 c = fit(persp(rot(model.vert(i, 2), M_PI / 6)));
        TGAColor rand_color;
    ...
    for(int i = 0; i < model.nverts(); i ++) {
        vec3 p = fit(persp(rot(model.vert(i), M_PI / 6)));
        framebuffer.set(p.x, p.y, white);
    }

    framebuffer.write_tga_file("diablo_rgb_rot.tga");
    zbuffer.write_tga_file("dioblo_gray_rot.tga");
